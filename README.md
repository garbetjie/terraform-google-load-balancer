...
| default_backend       | Backend type and index to select as the default backend. Must be a tuple with the first index being one of `bucket` or `service`, and the second the index of the bucket or service designated as the default. | tuple(string, number) | n/a     | Yes      |
